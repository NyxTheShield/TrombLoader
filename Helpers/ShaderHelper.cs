...
namespace TrombLoader.Helpers
{
    public class ShaderHelper
    {
        ...
        public Dictionary<string, Shader> ShaderCache { get; }

        public Dictionary<string, Shader> BaseGameShaderCache { get; } = new();
        ...
        public List<string> BaseGameShaderNames = new()
        {
        ...
        private void LoadBaseGameShaders()
        {
            ...
            {
                if (BaseGameShadersFamiliesToNotLoad.Any(e => baseGameShaderName.ToLower().StartsWith(e.ToLower())))
                    continue;
        ...
        public Dictionary<string, Shader> LoadShaderBundleFromPath(string path, RuntimePlatform? platform = null)
        {
            ...
            {
                foreach (var fileExtension in shaderBundleFileExtensions)
                {
                    var files = Directory.GetFiles(Path.GetDirectoryName(path), fileExtension,
                        SearchOption.TopDirectoryOnly);
                    foreach (var file in files)
                    {
                        if ((file.ToLower().Contains("macos") || file.ToLower().Contains("osx")) &&
                            !bundlePaths.Contains(file)) bundlePaths.Add(file);
                    }
                }
            }
            ...
            foreach (var bundlePath in bundlePaths)
            {
                ...
                if (asset != null)
                {
                    foreach (var renderer in asset.GetComponentsInChildren<Renderer>())
                    {
                        ...
                        // unnecessary to cache shaders that already exist in base game, as we can just use those
                        if (shader == null || shaderMap.ContainsKey(shader.name) ||
                            BaseGameShaderCache.ContainsKey(shader.name)) continue;
                ...
                {
                    Plugin.LogWarning($"Missing _Shaders.prefab file in bundle {bundlePath}!");
                }
